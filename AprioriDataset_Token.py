...
for value in values:
    ...
    begin = -1
    read = []
    print("|", str("Calculate "+value+" ").center(50), "|")
    ...
    for i in tqdm(range(start+1, addrDF.shape[0])):
        ...
        """
        # 单位wei转换成ETH
        if type == 'USDC' or type =='USDT':
            dec = -6
        if type =='WBTC' or type == 'cDAI':
            dec = -8
        if type == 'ETH' or type == 'DAI':
            dec = -18
        txdf['value'] = txdf['value'].str.slice(stop=dec) + '.' + txdf['value'].str.slice(start=dec)
        # 字符串类类型转换
